...
sum_tab %>%
  ...
  mutate(valid_pct = scales::percent(valid_pct)) %>%
  setNames(c(
    " ",
    rep(c("Mean", "Median", "SD", "Range", "N Valid"), 2),
    "Percentage Valid"
  )) %>%
  kable(format = "html", digits = 2) %>%
  kable_styling(bootstrap_options = "striped", full_width = F, font_size = 14) %>%
  add_header_above(c(" " = 1, "Male" = 5, "Female" = 5, " ")) %>%
  footnote(
...
**Fit univariate regression models with covariates and independent variables.**

The first dependent variables for analysis 1 are:

*From manuscripts/OSF.md:*

> * Side effect severity indices (*SE_20*, *SE_15* and *SE_10*)
>
>   - We compute three side effect indices, which are the average number of side
>     effects across medications discounting those side effects having higher than
>     respectively 20%, 15% and 10% (*SE_20*, *SE_15* and *SE_10*) prevalence and
...
Below are the coefficients from univariate models where we fit a logistic
regression model for $Y = 0$ and a gamma GLM with log link for $Y > 0$.
Inferential statistics are given by the Wald test.

```{r}
kable_hurd(hm_20_uni)
```

```{r}
kable_hurd(hm_15_uni)
```

```{r}
kable_hurd(hm_10_uni)
```

**Below are the multivariate models**. I looked at the diagnostics for
these multivariate models and they all seem to fit quite well with regards
to residual distributions. There seems to be, however, some
multicollinearity going on with those anxiety and depression related
variables.
...
It's interesting though this paper "Modelling Non-negative Data with
Clumping at Zero: A Survey" [@62XAWTGG#Y_A_2002_Modeling] suggests that it
is also plausible to apply binning to data whilst treating zeroes as an
independent category and fit proportional odds models (which I'm using
below not for continuous but for categorical data).

The following two dependent variables in analysis 1 are ordinal variables
...
I cannot run univariate regressions with the package I'm using for this
model, because it does not allow specifying only nominal effects - perhaps
the package author thinks that with only nominal effects the model isn't
"ordinal" any more in a sense that the direction of the effect might not
stay the same across levels? It's likely that there are other packages that
can do this though. Would this be necessary to do?

### Model 2

The second dependent variable for analysis 1 is:

*From manuscripts/OSF.md:*

> - Self-report side effect severity rating (1-5 Likert scale)
>
>   Item in questionnaire: *"Overall, how would you rate the side-effects of
...
The third dependent variable for analysis 1 is:

*From manuscripts/OSF.md:*

> * Likelihood of intolerance
>
>   - Computed by averaging treatment intolerance (binary) across medications.

The measure originally collected in the questionnaire is:

> - Treatment intolerance for each antidepressant (binary)
>
>   Example item in questionnaire: *"Did you have to stop taking any of these
>   medication(s) because of side effects? - Citalopram (e.g. Cipramil)"*
...
For confirmatory analysi 2 we seek to determine if effectiveness is
assocaited with side effect severities.

### Model 1

The first dependent variable for confirmatory analysis 2 is the average effectiveness index.
...
The second dependent variable for confirmatory analysis 2 is the average effectiveness index.
...
I only included the models that you saw last time and those that I think we
will most likely run in the end. There are more models I proposed in the
OSF, but since we have not decided on a multiple testing procedure yet I'm
not sure if I should run them now. The functions for them are written so
once we have the right variables running them should be instantaneous.
